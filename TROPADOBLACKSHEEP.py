...
# Função para decodificar URLs ofuscados
...
class TelaChecker(Screen):
    def __init__(self, **kwargs):
        ...
        self.url_menu_json = decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/main/version.json')
        self.layout = BoxLayout(orientation='vertical', padding=15)
        ...
        self.imagem = self.baixar_e_exibir_imagem(decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/main/scriptbyblacksheep.png'))
    ...
    def _verificar_atualizacao_thread(self):
        try:
            ...
            if dados['version'] != self.versao_local:
                Clock.schedule_once(lambda dt: self.mostrar_popup_nova_versao(dados['version'], dados['script_url'], dados['script_hash']), 0)
            else:
    ...
    def mostrar_popup_nova_versao(self, nova_versao, script_url, script_hash):
        content = BoxLayout(orientation='vertical', padding=15)

        imagem = self.baixar_e_exibir_imagem(decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/refs/heads/main/atualizacao.png'))
        ...
        btn_atualizar.bind(on_press=lambda x: self.baixar_script(script_url, script_hash))
    ...
    def baixar_script(self, url, script_hash):
        Thread(target=self._baixar_script_thread, args=(url, script_hash)).start()

    def _baixar_script_thread(self, url, script_hash):
        ...
        try:
            ...
            # Verifica o hash do arquivo baixado
            if calcular_hash(caminho_arquivo) == script_hash:
                Clock.schedule_once(lambda dt: self.atualizar_status('Script atualizado. Reiniciando...'), 0)
                os.execv(sys.executable, ['python'] + sys.argv)
            else:
                Clock.schedule_once(lambda dt: self.atualizar_status('Erro: O hash do script não coincide!'), 0)

        except Exception as e:
            Clock.schedule_once(lambda dt: self.atualizar_status(f'Erro ao baixar o script: {str(e)}'), 0)

    def carregar_dados_github(self):
        url = decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/main/Menu.json')
        try:
            resposta = requests.get(url)
            resposta.raise_for_status()
            dados = resposta.json()
    ...
    def mostrar_popup_telegram(self, instance):
        ...
        # URL do Telegram
        telegram_url = decodificar_url('https://t.me/BLACKSHEEP_B')  # Substitua pelo link do seu canal

        imagem = self.baixar_e_exibir_imagem(decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/refs/heads/main/redesocial.png'))
    ...
    def mostrar_popup_sobre(self, instance):
        ...
        # Resumo pequeno com cor vermelha
        resumo = "Este aplicativo foi desenvolvido\npara facilitar o acesso a scripts e recursos\nrelacionados ao IPTV.\n\n\nScript desenvolvido por: BLACKSHEEP_B\nCOPYRIGHT © todos os direitos TROPADOBLACKSHEEP"
        resumo_label = Label(text=resumo, size_hint_y=None, height=200, color=(1, 0, 0, 1))  # Cor vermelha
        content.add_widget(resumo_label)

        # Adicionando a imagem
        imagem = self.baixar_e_exibir_imagem(decodificar_url('https://raw.githubusercontent.com/Checkstatusiptv/Checkm3u/refs/heads/main/GITHUB-29-09-2024.png'))
        content.add_widget(imagem)
        ...
        self.popup = Popup(title='Sobre o Aplicativo', content=content, size_hint=(0.7, 0.6))
        self.popup.open()
